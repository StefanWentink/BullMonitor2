    ...
    public static class SunRisetUtilities
    {
            ...
            out double tsunset)
        {
            SunriseSunset(
                date.Year,
                date.Month,
                date.Day, lng, lat, SunriseSunsetAltitude, true, out tsunrise, out tsunset);
        }
        /// <summary>
        /// Compute sunrise/sunset times UTC
        /// </summary>
        ...
        /// <param name="tsunrise">The computed sunrise time (in seconds)</param>
        /// <param name="tsunset">The computed sunset time (in seconds)</param>
        public static (DateTimeOffset sunrise, DateTimeOffset sunset) SunriseSunset(
            DateTimeOffset date,
            double lat,
            double lng,
            TimeZoneInfo timeZoneInfo)
        {
            var dutchDate = date.ToTimeZone(timeZoneInfo);

            var offset = date.SetToTimeOfDay(6, 0, 0, timeZoneInfo).Offset;
            ...
            var sunrise = dutchDate + offset + TimeSpan.FromHours(_tsunrise);
            var sunset = dutchDate + offset + TimeSpan.FromHours(_tsunset);
